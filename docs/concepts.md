...
|---|---|---|
| `CONCEPT:OBJ-1.0` | Multi-Backend Store Abstraction | The `ObjectStoreBackend` protocol in `api/api_client_base.py`: one bucket-agnostic contract every provider implements, validated by a single conformance suite (`tests/test_backend_conformance.py`) |
