...
`objectstore-mcp` exposes its MCP server (console script `objectstore-mcp`) four ways. Pick the row that
...
Caddy reverse-proxies `http://objectstore-mcp.arpa` to the container's `:8000`
streamable-http listener; `http://objectstore-mcp.arpa/health` returns
