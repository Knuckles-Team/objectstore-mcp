"""Every CONCEPT:OB-OS.governance.obj-x marker in code must be registered in docs/concepts.md."""

import re
from pathlib import Path

REPO = Path(__file__).resolve().parents[1]
MARKER = re.compile(r"CONCEPT:OB-OS.governance.obj\.\d+")
...
def test_code_markers_are_registered_in_docs():
    ...
    assert in_code, "Expected CONCEPT:OB-OS.governance.obj-x markers in objectstore_mcp/"
...
def test_eco_bridge_and_root_concept_documented():
    ...
    assert "CONCEPT:OB-OS.governance.every-provider-implements-same" in registry
    assert "CONCEPT:OB-OS.governance.every-provider-implements-same" in collect_code_markers()
